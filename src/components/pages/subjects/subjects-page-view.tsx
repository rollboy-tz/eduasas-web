...
import { EduMainModal, showConfirm } from "@/components/modals";
...
export function SubjectsPageView() {
    ...
    const { setWorkspaceHeader, setIsSaving } = useWorkspace();

    useEffect(() => {
        setWorkspaceHeader({ title: "Subjects" });
    }, [setWorkspaceHeader]);
    ...
    //Selection
    const [selectedSugeddtionIs, setSelectedSuggestinId] = useState<string[]>([])

    //Subject registration
    const handleReegisterSuggested = async () => {
        if (selectedSugeddtionIs.length === 0) return;
        setIsSaving(true);
        const toastId = toast.show({ message: `Adding ${selectedSugeddtionIs.length} subject(s)`, type: "loading" })
        const data = selectedSugeddtionIs.map((sub) => ({ subjectId: sub }))
        try {
            const result = await registerSubjects(data);
            if (result.status === "success") {
                const message = "Subject(s) added successfuly"
                toast.show({ message, type: "success" })
            }

        } catch (err) {
            if (isApiError(err)) {
                const message = err.message || "Failed to add subject(s)"
                toast.show({ message, type: "success" })
            }
        } finally {
            toast.dismiss(toastId);
            setIsSaving(false);
            setSelectedSuggestinId([]);
        }
    }
    ...
    return (
        <>
            <div className="w-full max-w-7xl mx-auto  md:w-full h-[calc(100vh-80px)] flex flex-col">

                {/* Tab Navigation (Mobile) */}
                <div className="flex md:hidden w-full gap-1 mb-1">
                    <button
                ...
                <div className="w-full overflow-hidden relative">
                    ...
                    >
                        {/* 1. REGISTERED COLUMN */}
                        <div className="h-full w-[50%] md:flex-1 md:w-full flex flex-col bg-white/90 border border-border rounded-md overflow-hidden">
                            <div className="p-4 border-b border-border font-bold">Registered Subjects</div>

                            <div className="flex-1 overflow-y-auto p-4 text-muted-foreground">
                                {
                                    registaredSubjects.length > 0 ? (
                                        <SubjectsContainer onSelect={(id) => console.log(id)} registeredSubjects={registaredSubjects} />
                                    ) : (
                                        <EmptyState
                        ...
                        <div className={`flex-col w-[50%] md:max-w-sm bg-card rounded-md border border-border h-full bg-white/90
                            ...
                            <div className="flex-1 overflow-y-auto bg-card/5">
                                ...
                                <SubjectSuggestionContainer suggestedSubjects={suggestedSubjects}
                                    onSelect={(selected) => setSelectedSuggestinId(selected)}
                                />
